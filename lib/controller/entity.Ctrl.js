...
getVoterContactList: function(req, res)
    {
        console.log(req.decoded.listnos)
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                if(req.decoded.listnos != '')
                {
                    var listnos = 'WHERE'+req.decoded.listnos
                }
                else
                {
                     var listnos = req.decoded.listnos
                }
                con.query('SELECT *,(SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no, (SELECT COUNT(*) - 50 FROM voters '+listnos+') as totalcontacts FROM voters '+listnos+' ORDER BY id '+req.params.sort+' LIMIT 50', function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
    ...
    FilterNearestResult: function(req, res)
    {
         if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                   if(req.decoded.listnos != '')
                   {
                       var listnos = 'AND'+req.decoded.listnos
                   }
                   else
                   {
                        var listnos = req.decoded.listnos
                   }
                con.query('SELECT *,(SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no, (SELECT COUNT(*) - 50 FROM voters WHERE '+req.body.field+' LIKE "%'+req.body.searchinput+'%" '+listnos+') as totalcontacts FROM voters WHERE '+req.body.field+' LIKE "%'+req.body.searchinput+'%" '+listnos+' ORDER BY id ASC LIMIT 50', function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
    ...
    ImportVoterContactDetails: function(req, res)
    {
        if (req.decoded.success == true) {   
                ...
                req.body.map(function(value, index){
                    ...
                    if(value[6] != undefined){value[6].replace(/[^a-zA-Z0-9]/g, '')}else{value[6] = ""}
                    if(value[7] != undefined){value[7].replace(/[^a-zA-Z0-9]/g, '')}else{value[7] = ""}
    ...
    getVoterContactListPagination: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
             ...
             con.query('SELECT *, (SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no,(SELECT COUNT(*) - 50 FROM voters '+searchFilter+''+listnos+') as totalcontacts FROM voters '+searchFilter+''+listnos+' ORDER BY id ASC LIMIT '+skiplimit, function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
    ...
    getListRecordDetails:function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                con.query('SELECT listnos.id,listnos.listno,listnos.oldlistno,listnos.modifieddate, (SELECT users.name FROM users WHERE users.id = listallocation.userid) as name,listallocation.userid FROM listnos INNER JOIN listallocation ON listallocation.listid = listnos.id WHERE listnos.id = '+req.params.listid, function(err,result){
                    if(err)
                    {
                        res.send({
                            status: 1,
