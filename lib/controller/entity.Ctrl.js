...
getVoterContactList: function(req, res)
    {
        ...
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                con.query('SELECT *,(SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no, (SELECT COUNT(*)  FROM voters '+listnos+') as totalcontacts FROM voters '+listnos+' ORDER BY id '+req.params.sort+' LIMIT 50', function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
    ...
    FilterNearestResult: function(req, res)
    {
         if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                con.query('SELECT *,(SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no, (SELECT COUNT(*)  FROM voters WHERE '+req.body.field+' LIKE "%'+req.body.searchinput+'%" '+listnos+') as totalcontacts FROM voters WHERE '+req.body.field+' LIKE "%'+req.body.searchinput+'%" '+listnos+' ORDER BY id ASC LIMIT 50', function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
        ...
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                if(req.decoded.listnos != '')
                {
                    if(searchFilter == '')
                    {
                        var listnos = ' WHERE'+req.decoded.listnos+' OR createdby = '+req.decoded.logedinuser.id
                    }
                    else
                    var listnos = 'AND createdby = '+req.decoded.logedinuser.id+' OR '+req.decoded.listnos
                }
             ...
             con.query('SELECT *, (SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no,(SELECT COUNT(*) FROM voters '+searchFilter+''+listnos+') as totalcontacts FROM voters '+searchFilter+''+listnos+' ORDER BY id ASC LIMIT '+skiplimit, function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
