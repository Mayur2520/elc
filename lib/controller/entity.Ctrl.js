...
=======
>>>>>>> b8d63451f8e5fcd159a4d2668f3e5f6abc500fd0
    ...
    FilterNearestResult: function(req, res)
    {
         if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                con.query('SELECT *, (SELECT COUNT(*) - 50 FROM voters WHERE '+req.body.field+' LIKE "%'+req.body.searchinput+'%") as totalcontacts FROM voters WHERE '+req.body.field+' LIKE "%'+req.body.searchinput+'%" ORDER BY id DESC LIMIT 500', function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
