...
getVoterContactList: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                con.query('SELECT *,(SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no, (SELECT COUNT(*) - 50 FROM voters) as totalcontacts FROM voters ORDER BY id DESC LIMIT 50', function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
    ...
    FilterNearestResult: function(req, res)
    {
         if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                con.query('SELECT *,(SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no, (SELECT COUNT(*) - 50 FROM voters WHERE '+req.body.field+' LIKE "%'+req.body.searchinput+'%") as totalcontacts FROM voters WHERE '+req.body.field+' LIKE "%'+req.body.searchinput+'%" ORDER BY id DESC LIMIT 50', function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
    ...
    getVoterContactListPagination: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
             ...
             con.query('SELECT * (SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no,(SELECT COUNT(*) - 50 FROM voters '+searchFilter+') as totalcontacts FROM voters '+searchFilter+' ORDER BY id DESC LIMIT '+skiplimit, function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
    ...
    SaveListDetails: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                   if(req.body.id)
                   {
                    con.query('UPDATE listnos set oldlistno = (SELECT `listno` FROM `listnos` WHERE `id` = ?),modifieddate = CURDATE() WHERE id = ?;UPDATE listnos set listno = ? WHERE id = ?',[req.body.id,req.body.id,req.body.listno, req.body.id], function(err,result){
                        if(err)
                        {
                            res.send({
                                status: 1,
