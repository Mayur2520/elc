        ...
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                if(req.decoded.listnos != '')
                {
                    ...
                    var sortfied = 'ORDER BY listno , srno ASC ';
                }
                else
                {
                     var listnos = req.decoded.listnos;
                     var sortfied = 'ORDER BY id '+req.params.sort+' ';
                }

                con.query('SELECT *,(SELECT listnos.listno FROM listnos WHERE listnos.id = voters.listno) as list_no, (SELECT COUNT(*)  FROM voters '+listnos+') as totalcontacts FROM voters '+listnos+' '+sortfied+' LIMIT 50', function(err,result){
                    if(err)
                    {
                        console.log(err)
                        res.send({status:1, message:"Something went wrong"})
